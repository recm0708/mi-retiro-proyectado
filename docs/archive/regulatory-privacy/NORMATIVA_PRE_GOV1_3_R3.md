...
- **parámetro versionado**: valor almacenado en `regulations/*.json` con fecha y fuente;
...
`regulations/sebd.json` almacena la tabla mensual de factores de reducción utilizada por el motor. La fuente reglamentaria se mantiene en la sección **Reglamento para el Cálculo de Prestaciones Económicas** de la página oficial de Prestaciones Económicas de la CSS.
...
La tabla de factores se almacena en `regulations/sucgs.json`. La ley dispone actualización periódica, por lo que el motor no debe tratarlos como inmutables fuera de su versión.
...
Los enlaces normativos continúan versionados en `regulations/*.json`. La interfaz no debe duplicar URLs en JavaScript. `app/services/fuentes_normativas.py` transforma esos metadatos en un catálogo humano para `/metodologia`, mientras la trazabilidad resuelve los IDs internos a títulos y enlaces oficiales.
