...
- Evidencia de aceptación: PR #101 / merge `6f4266d`; promoción PR #102 / `main` `dfb7dc60cf81951c701c126d6fecbcfdbca7aa7b`; revalidación post-merge con `pytest` 1172 passed / 5615 subtests, Markdown 158/158, auditor de identificadores, `pip check`, compilación Python, sintaxis JavaScript, `git diff --check`, CI, gobernanza, Markdown Audit y CodeQL en `success`.
...
- Publicación final: tag anotado y firmado `v0.1.16.05-beta`, objeto `4b5902bf7a3d2b94fcad8a426652d7ad8b77a32c`, firma SSH válida en GitHub, workflow `Git Tag Signature Verification` #14 en `success` y GitHub Release prerelease ID `378095836` publicado; `v0.1.15.04-beta` permanece preservado e inmutable.

## Promoción G115/E04 — post-merge de DOC.1 R4

- Estado aceptado: G115/E04 (`0.1.15.04-beta`).
- Evidencia de aceptación: PR #96 / merge `9f51229`, 1114 pruebas post-merge, Markdown 149/149, auditor de identificadores en `OK`, `pip check`, compilación Python, sintaxis JavaScript y `git diff --check` limpios; promoción final PR #97 / merge `87fa8e9`, revalidada con 1116 pruebas.
- Esta sincronización materializa G115 ya aceptado y **no consume G116**.
- La reserva G116/E02 para REL.GOV.1 R2 fue la planificación vigente al publicar G115; DOC.1 R5 reasigna posteriormente el Global todavía no consumido a G116/E05 (`0.1.16.05-beta`). REL.GOV.1 R2 permanece a continuación sin Global preasignado.
- Promoción final: PR #97 / merge `87fa8e9`; tag anotado y firmado `v0.1.15.04-beta`, objeto `21a27a0b1e09e86136ae15042796576013df130d`, workflow de firma y GitHub Release prerelease publicados; `v0.1.14.01-beta` permanece preservado.
