...
class TestUX5R7DocumentationClosure(unittest.TestCase):
    ...
    def test_matriz_preserva_orden_ux5_ux6_persistencia(self):
        text = (
            ROOT / "docs/governance/pre-1-0-pending-matrix.md"
        ).read_text(encoding="utf-8")

        ux5 = text.index("**UX.5 R6**")
        ux6 = text.index("**UX.6 R1–R8**")
        persist = text.index("**PERSIST.1 R1**")
    ...
    def test_assets_locales_tienen_revision(self):
        ...
        self.assertIn("path='/js/simulation_mode.js'", simulation)
        self.assertIn("path='/js/assisted_flow.js'", simulation)
