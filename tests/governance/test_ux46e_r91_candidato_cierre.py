...
class TestUx46eR91CandidatoCierre(unittest.TestCase):
    ...
    def test_security_distingue_candidato_y_lineas_historicas(self):
        ...
        version = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
        self.assertIn(f"| `{version}` | Beta vigente ", texto)
    ...
    def test_regresiones_historicas_no_congelan_version_0_0_24(self):
        rutas = (
            ROOT / "tests/governance/test_gov18_cierre_gobierno.py",
            ROOT / "tests/governance/test_identidad_publica_github_pre_r8.py",
            ROOT / "tests/repository/test_ux46e_estandar_runtime.py",
            ROOT / "tests/portals/asegurado/test_ux46e_r81_editable_provenance.py",
            ROOT / "tests/regression/test_ux46e_r8_cierre_funcional.py",
        )
        patron = re.compile(
