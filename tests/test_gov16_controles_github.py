...
class TestGov16ControlesGithub(unittest.TestCase):
    ...
    def test_issue_forms_separan_error_mejora_y_seguridad(self):
        b=(GH/'ISSUE_TEMPLATE/bug_report.yml').read_text(encoding='utf-8'); f=(GH/'ISSUE_TEMPLATE/feature_request.yml').read_text(encoding='utf-8'); c=(GH/'ISSUE_TEMPLATE/config.yml').read_text(encoding='utf-8')
        self.assertIn('Reporte de error',b); self.assertIn('Solicitud de mejora',f); self.assertIn('SECURITY.md',b); self.assertIn('blank_issues_enabled: false',c); self.assertIn('/security/policy',c)
    ...
    def test_documento_auditoria_registra_controles(self):
        ...
        for e in ('Pull Request obligatorio','Python 3.13','Python 3.14','Auditoría de gobernanza','force push','Dependency graph','Dependabot alerts'): self.assertIn(e,t)
    ...
    def test_archivos_nuevos_limpios(self):
        ps=[ROOT/'SECURITY.md',DOCS/'AUDITORIA_GITHUB.md',GH/'ISSUE_TEMPLATE/bug_report.yml',GH/'ISSUE_TEMPLATE/feature_request.yml',GH/'ISSUE_TEMPLATE/config.yml',GH/'pull_request_template.md',GH/'workflows/auditoria-gobernanza.yml']
        for p in ps:
