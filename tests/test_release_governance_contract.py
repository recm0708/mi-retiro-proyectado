...
class TestReleaseGovernanceContract(unittest.TestCase):
    ...
    def test_contrato_actual_deriva_titulo_g109(self):
        result = self.run_contract("--json")
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        data = json.loads(result.stdout)
        self.assertEqual("0.1.09.01-beta", data["version"])
        self.assertEqual("v0.1.09.01-beta", data["tag"])
        self.assertEqual(109, data["global_revision"])
    ...
    def test_tag_debe_coincidir_con_version(self):
        ok = self.run_contract("--check-tag", "v0.1.09.01-beta")
        self.assertEqual(0, ok.returncode, ok.stdout + ok.stderr)
        bad = self.run_contract("--check-tag", "v0.1.10.01-beta")
        self.assertNotEqual(0, bad.returncode)
        self.assertIn("Tag inválido", bad.stdout)

    def test_titulo_debe_ser_canonico(self):
        title = "Mi Retiro Proyectado v0.1.09.01-beta — G109/E01"
        ok = self.run_contract("--check-title", title)
        self.assertEqual(0, ok.returncode, ok.stdout + ok.stderr)
        bad = self.run_contract("--check-title", "v0.1.09.01-beta")
        self.assertNotEqual(0, bad.returncode)
    ...
    def test_documentacion_operativa_cubre_script_y_publicacion(self):
        ...
        self.assertIn("next_candidate_block = REL.GOV.1", validation)
        self.assertIn("## 7.1. Tags y GitHub Releases", github)
        self.assertIn(".github/release.yml", github)

    def test_g110_queda_reservado_para_rel_gov_1(self):
        ledger = json.loads(
