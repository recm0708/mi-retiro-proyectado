...
class TestAccesibilidadTemas(unittest.TestCase):
    """Protege las decisiones visibles introducidas en la fase UX.2."""

    @classmethod
    def setUpClass(cls):
        cls.cliente = TestClient(app)

    def test_selector_de_tema_ofrece_cuatro_modos(self):
        respuesta = self.cliente.get("/")

        self.assertEqual(respuesta.status_code, 200)
        self.assertIn('id="selector-tema"', respuesta.text)
        self.assertIn('value="system"', respuesta.text)
        self.assertIn('value="light"', respuesta.text)
        self.assertIn('value="dark"', respuesta.text)
        self.assertIn('value="contrast"', respuesta.text)
    ...
    def test_footer_es_compacto_y_mantiene_aviso_legal(self):
        ...
        self.assertIn('class="container footer-compact"', respuesta.text)
        self.assertIn("Mi Retiro Proyectado.</strong>", respuesta.text)
